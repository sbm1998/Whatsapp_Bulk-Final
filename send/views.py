...
def welcome_view(request):
    if not check_user(request):
        return render(request,r"send\please_login.html")
    whatsapp_login()
...
def list_phone_number_view(request):
    if not check_user(request):
        return render(request,r"send\please_login.html")
    obj=Whatsapp_Numbers.objects.filter(user=request.user)
    return render(request,r"send\phone_numbers.html",{"numbers":obj})

def number_delete_view(request,id):
    if not check_user(request):
        return render(request,r"send\please_login.html")
    obj=Whatsapp_Numbers.objects.filter(pk=id)
    if not check_same(request,obj):
...
def send_view(request):
    ...
    if request.method=="POST":
        ...
        lst=Whatsapp_Numbers.objects.filter(user=request.user).values_list("number")
        numbers=[]
        for number in lst:
            number=str(number)
            a=number[3:15]
    ...
    if request.method == "POST":
        ...
        if ext != ".csv":
            ...
            if obj.is_valid(): 
                ...
                for number in lst:
                    whats_obj=Whatsapp_Numbers(number=number,user=request.user)
                    whats_obj.save()
                os.remove(address)
...
def check_same(request,obj):
    if str(obj[0].user)==str(request.user):
        return True
    else:
        return False
